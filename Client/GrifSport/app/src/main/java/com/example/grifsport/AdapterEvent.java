...
public class AdapterEvent extends  RecyclerView.Adapter<AdapterEvent.HolderOneNote> implements Filterable  {
    ...
    private com.example.grifsport.databinding.ActivityOneEventBinding binding;
    java.util.ArrayList <Integer> ListId = new ArrayList<>();
    private  FilterEvent filter;

    //private  FilterPdfUser filter;
    //фрагмент одной кв.
    public AdapterEvent(Context context, java.util.ArrayList<EventResponse> pdfArrayList, String access,int profile_id) {
    ...
    @Override
    public HolderOneNote onCreateViewHolder( ViewGroup parent, int viewType) {
        binding = ActivityOneEventBinding.inflate(LayoutInflater.from(context),parent,false);

        this.context=parent.getContext();
        return new HolderOneNote(binding.getRoot());
    }
    ...
    @Override
    public void onBindViewHolder( HolderOneNote holder, int position) {
    ...
    public Filter getFilter() {
        if(filter==null){
            filter = new FilterEvent(filterList,this);
        }
        return filter;
    }


    class HolderOneNote extends RecyclerView.ViewHolder{
       ImageView circleImageView;
        TextView NoteDescription,NoteDate,NoteAssesment;
        CardView cardView;
        ImageButton buttonTv;
        public HolderOneNote( View itemView) {
            super(itemView);
